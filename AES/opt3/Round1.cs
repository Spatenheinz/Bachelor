using System;
using SME;
using static opt3.AESConfig;

namespace opt3
{
    [ClockedProcess]
    class Round1 : SimpleProcess {
        [InputBus] public IPlainText PlainText;
        [OutputBus] public axi_r axi_in = Scope.CreateBus<axi_r>();
        [OutputBus] public IState Out = Scope.CreateBus<IState>();
        [InputBus] public axi_r axi_out;

        private uint[] expandedKey128 = new uint[8];

        bool was_valid = false;
        bool was_ready = false;
        protected override void OnTick() {
            if (was_ready && PlainText.ValidKey) {
                Expand128(PlainText.Key);
                Console.WriteLine("expanding");
            } else if (was_ready && PlainText.ValidBlock) {
                Console.WriteLine("calculating");
                Encrypt128();
                Out.Valid = was_valid = true;
            } else {
                Out.Valid = was_valid = was_valid && !axi_out.ready;
            }
            axi_in.ready = was_ready = !was_valid;
        }

        private uint SubWord(uint x) {
            return
                   ((uint)S[0xff & (x>> 24)] << 24) |
                   ((uint)S[0xff & (x>> 16)] << 16) |
                   ((uint)S[0xff & (x>> 8)] << 8) |
                   ((uint)S[0xff & x]);
        }

        private void Expand128(IFixedArray<byte> key) {
            for (int i = 0; i < N_KEY_128<<2; i+=4) {
                Out.Key[i>>2] = expandedKey128[i>>2] = ((uint)key[i] << 24) |
                                        ((uint)key[i+1] << 16) |
                                        ((uint)key[i+2] << 8) |
                                        ((uint)key[i+3]);
            }
        }
        private uint LeftRotate(uint x, int k) {
            return ((x << k) | ((x >> (32 - k)) & 0xff));
        }


        private void Encrypt128() {

            Out.Block[0] = (((uint)PlainText.block[0] << 24) | ((uint)PlainText.block[1] << 16) | ((uint)PlainText.block[2] << 8) | (uint)PlainText.block[3]) ^ expandedKey128[0];
			Out.Block[1] = (((uint)PlainText.block[4] << 24) | ((uint)PlainText.block[5] << 16) | ((uint)PlainText.block[6] << 8) | (uint)PlainText.block[7]) ^ expandedKey128[1];
			Out.Block[2] = (((uint)PlainText.block[8] << 24) | ((uint)PlainText.block[9] << 16) | ((uint)PlainText.block[10] << 8) | (uint)PlainText.block[11]) ^ expandedKey128[2];
			Out.Block[3] = (((uint)PlainText.block[12] << 24) | ((uint)PlainText.block[13] << 16) | ((uint)PlainText.block[14] << 8) | (uint)PlainText.block[15]) ^ expandedKey128[3];
        }

		static readonly byte[] S = new byte[] {
			
		};
    }
}
